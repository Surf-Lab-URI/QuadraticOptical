...
from ._provenance import forbidden_input_keys, source_hashes, FITTING_SOURCES
...
def verify_image_only(data, label):
    ...
    forbidden = forbidden_input_keys(data) if label == 'Image inputs' else [
        k for k in data if k.startswith('classical') or k.startswith('supplied_d') or
        k in ('delta_x', 'delta_z', 'delta_x1', 'delta_z1')]
    if forbidden:
...
def verify_tracks(path, expected_input_hash):
    ...
    if 'complete' in data and not bool(data['complete']):
        raise ValueError('Automatic tracks are not complete.')
    return data
...
def empty_model(points, radius, order, margin, inputs, common, run_signature, stage):
    ...
    data = dict(points=points, params=np.full((n, 2, k), np.nan),
                ...
                tracking_sha256=common['source_sha256']['tracking.py'],
                fit_driver_sha256=common['source_sha256']['fit_fields.py'],
...
def reverse_stage(directory, inputs, common, workers, checkpoint_every):
    ...
    # A reverse-only resume must also reject a forward fit produced by older
...
def run(directory, workers=1, stages=None, checkpoint_every=512):
    """Run/resume local priors, four forward fits and one reciprocal fit."""
    directory = Path(directory).resolve()
    if workers < 1 or checkpoint_every < 1:
        raise ValueError('workers and checkpoint_every must be positive.')
    stages = list(stages) if stages is not None else ['seeds'] + list(VARIANTS) + ['reverse']
    if any(stage not in ['seeds'] + list(VARIANTS) + ['reverse'] for stage in stages):
    ...
    common = dict(settings=SETTINGS, input_sha256=file_hash(input_path),
                  ...
                  ptv_model_sha256=file_hash(Path(__file__).resolve().with_name('ptv_model.py')),
                  source_sha256=source_hashes(FITTING_SOURCES))
    ...
    seeds = None
    if 'seeds' in stages or any(stage in VARIANTS for stage in stages):
        seeds = seed_stage(directory, inputs, common, checkpoint_every)
    ...
    for stage in stages:
        if stage in VARIANTS:
            ...
            report[stage] = reverse_stage(directory, inputs, common, workers,
                                          checkpoint_every)
            atomic_json(report_path, report)
