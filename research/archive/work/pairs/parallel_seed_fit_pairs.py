...
Example: python work/pairs/parallel_seed_fit_pairs.py --pairs 80 --workers 4
...
fit_pairs.py, since the numerical model and its inputs are unchanged.
"""
import os
...
_ORIGINAL_SEED_STAGE = fit_pairs.seed_stage
_ORIGINAL_RUN_PAIR = fit_pairs.run_pair
...
def parallel_seed_stage(directory, inputs, common, checkpoint_every, max_workers=4):
    ...
    try:
        ...
        fit_pairs.PTVModel = original_constructor
        for model in proxies:
            model.close(terminate=failed)


def run_pair(pair, args):
    original_seed_stage = fit_pairs.seed_stage
    ...
    fit_pairs.seed_stage = seed_stage
    try:
        return _ORIGINAL_RUN_PAIR(pair, args)
    finally:
        fit_pairs.seed_stage = original_seed_stage


def main():
    fit_pairs.run_pair = run_pair
    fit_pairs.ThreadPoolExecutor = ForkProcessExecutor
    return fit_pairs.main()
