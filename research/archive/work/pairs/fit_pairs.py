"""Fresh-pair conservative local registration, with atomic resumable stages.

No earlier pair's coefficients, tracks, or manual references are used. The
...
SETTINGS = dict(schema_version=SCHEMA_VERSION, fresh_pair=True,
...
def empty_model(points, radius, order, margin, inputs, common, run_signature, stage):
    ...
    data = dict(points=points, params=np.full((n, 2, k), np.nan),
                ...
                fresh_pair=True, manual_data_used=False)
