...
yg,xg=np.mgrid[10:191,50:401];points=np.column_stack([xg.ravel(),yg.ravel()]).astype(float);source_depth=points[:,1]-np.interp(points[:,0],np.arange(501),sa);use=source_depth>=10;points=points[use];Q=basis(points)
# Data include only fluid pixels, target mask uses smooth barrier on depth<6.
def solve(lam):
 ...
 for sigma,maxiter in [(1.6,1000),(.8,1600)]:
  ...
  def objective(flat):
   ...
   dep=dest[:,1]-np.interp(dest[:,0],np.arange(501),sb);invalid=np.minimum(dep-6,0);barrier=.02*np.mean(invalid*invalid);bargrad=.04*invalid/len(points)
...
if __name__=='__main__':
 ...
 (W/'spline_converged_metrics.json').write_text(json.dumps(report,indent=2))
