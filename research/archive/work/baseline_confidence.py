...
W=Path('work');D=Path('historical-user-files/Downloads');z=np.load(W/'metadata.npz');base=np.load(W/'baseline_reference.npz');F=base['flow'];xy=base['xy'];e=base['error'];depth=base['depth']
...
valid=(y>z['surfa'].ravel()[None,:]+7)&(yy>np.interp(xx,np.arange(501),z['surfb'].ravel())+7)&(xx>=0)&(xx<500)&(yy>=0)&(yy<500)
...
np.savez_compressed(W/'baseline_confidence.npz',forward=F,backward=G,fb_error=fb,local_ncc=ncc,valid=valid,manual_fb=pfb,manual_ncc=pncc,manual_accepted=good)
