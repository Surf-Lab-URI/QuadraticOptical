...
Run after root prepares image-only inputs:
  python work/image_only_cm/tracking.py --pair 80 --workers 4
Optional independent sensitivity branch, only if needed:
  python work/image_only_cm/tracking.py --pair 80 --bootstrap-radius 48
"""
import os
...
SOLVER = ROOT/'work'/'full_width'/'fit_local_cached.py'
EXECUTOR = ROOT/'work'/'pairs'/'process_executor.py'
...
def load_inputs(path):
    ...
    with np.load(path,allow_pickle=False) as z:
        forbidden=[k for k in z.files if k not in ['supplied_velocity_used'] and
                    (k.lower().startswith(('supplied_', 'classical',
                    'manual', 'velocity', 'previous_', 'old_', 'prior_', 'tracks')) or
                    k.lower() in ('disp','displacement','delta_x','delta_y','delta_z',
                                  'u','v','w','ux','uy','piv','flow'))]
        if forbidden:
...
class Runner:
    def __init__(self,args):
        self.args=args;self.base=ROOT/'work'/'image_only_cm'/str(args.pair)
        ...
        self.identity=dict(pair=args.pair,input_sha256=sha(path),settings=SETTINGS,
            source_sha256={str(p.relative_to(ROOT)):sha(p) for p in [Path(__file__),SOLVER,EXECUTOR]},
    ...
    def stage(self,name,points,task,pack,coarse=False):
        ...
        cls=ForkProcessExecutor if self.args.executor=='fork' else ThreadPoolExecutor
        with cls(max_workers=self.args.workers) as pool:
    ...
    def run(self):
        ...
        tracks=dict(points=pts,disp=fw['disp'],prior=fw['prior'],ncc=fw['ncc'],
...
def main():
    ...
    p.add_argument('--executor',choices=['fork','threads'],default='fork')
    ...
    a=p.parse_args()
    if a.workers<1 or a.chunk_size<1 or a.max_chunks<0:p.error('Invalid worker/chunk parameters.')
    return 0 if Runner(a).run() else 2
