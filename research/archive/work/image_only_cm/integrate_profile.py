...
def run(pair,interval_px=2.,depth_step_m=.0001,depth_batch=8):
    ...
    evaluator=ConservativeEvaluator(directory)
    ...
    arrays.update(sample_displacement_px=displacement,sample_surface_y_px=sx,
        ...
        DX=np.array(DX),DT=np.array(DT),physical_units_confirmed=np.array(False),
        surface_geometry_inferred=np.array(True),surface_trace_offset_px=np.array(12.),
    ...
    metadata=dict(pair=pair,source_directory=str(directory),source_hashes=hashes,
        ...
        physical_units_confirmed=False,surface_geometry_inferred=True,surface_offset_pixels=12.,
        masked_near_surface_note='The approximately12px image-masked strip cannot be recovered; h=0 is retained as missing, not filled.',
...
def self_test():
    ...
    (HERE/'integration_self_test.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
    if not report['passed']:raise SystemExit(1)
    return report

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--pair',type=int,choices=[80,100]);p.add_argument('--self-test',action='store_true')
    p.add_argument('--interval-px',type=float,default=2.);p.add_argument('--depth-step-m',type=float,default=.0001);p.add_argument('--depth-batch',type=int,default=8);a=p.parse_args()
    if a.self_test:self_test()
    elif a.pair is None:p.error('--pair is required unless --self-test is used')
    elif a.depth_step_m<=0 or a.depth_batch<1:p.error('Positive depth sampling and batch size required.')
    else:run(a.pair,a.interval_px,a.depth_step_m,a.depth_batch)
