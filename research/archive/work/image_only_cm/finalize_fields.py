...
from fit_fields import (WORK, read_npz, file_hash, scalar_text, atomic_npz,
...
def run_pair(pair, args):
    ...
    evaluator = ConservativeEvaluator(directory, batch_size=args.batch_size)
    ...
    metadata = dict(pair=str(pair), grid_count=n, fitting_grid_count=len(inp['points']),
                     ...
                     domain_note='Only source depths <=0.01/DX are reported; extra fitted halo supplies support.',
    ...
    summary = dict(pair=str(pair), method='Fresh image-only masked robust quadratic registration initialized by image-only automatic particle tracks.',
                    ...
                    DX=evaluator.DX, DT=evaluator.DT, physical_units_confirmed=False,
                    image_only=True, supplied_velocity_used=False,
                    ...
                    surface_note='Local geometric surface offset remains inferred; actual retained-image availability is applied independently.',
