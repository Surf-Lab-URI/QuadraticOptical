...
def main():
 ...
 for rep in range(6):
  ...
  for case in ['affine','quadratic','quadratic_boundary_glare']:
   ...
   for name,sym,cub in [('forward_cubic',False,True),('symmetric_bilinear',True,False),('symmetric_cubic',True,True)]:
    p,ss=solve(A,B,va,vb,CENTER,p0,r=r,symmetric=sym,cubic=cub,reg=.00005,maxiter=40)
  ...
  print('synthetic',rep+1,'/6 elapsed',round(time.time()-t,1),flush=True)
 summaries=[]
 for case in ['affine','quadratic','quadratic_boundary_glare']:
  for method in ['baseline','forward_cubic','symmetric_bilinear','symmetric_cubic']:
 ...
 result=dict(description=__doc__,replicates=6,particle_density=.04,blob_sigma_px=[.9,1.5],r=r,reg=.00005,true_translation=TRANS.tolist(),true_G=G.tolist(),true_quadratic_H=CURV.tolist(),boundary_center_depth_px=18,glare_sigma_px=6,summaries=summaries,individual=rows,seconds=time.time()-t)
 (HERE/'synthetic_metrics.json').write_text(json.dumps(result,indent=2));print(json.dumps(summaries,indent=2))
