...
Previous enlarged-domain coarse fits and all candidate/acceptance rows are frozen.
...
O=Path('work/full_width');O.mkdir(exist_ok=True);inp=np.load(O/'inputs.npz')
# Explicit image-only key selection prevents accidental use of validation fields.
A=inp['A'];B=inp['B'];rawA=inp['rawA'];rawB=inp['rawB'];va=inp['va'];vb=inp['vb'];sa=inp['surface_a'];sb=inp['surface_b'];points=inp['points'];origin=inp['origin0'];oldorigin=np.load('work/large_frame/inputs.npz')['origin0'];shift=oldorigin-origin;maxdepth=float(inp['max_depth']);supx=inp['supplied_dx'];supy=inp['supplied_dy'];target_gradient=np.gradient(B);target_mask_float=vb.astype(float)
base=np.load('work/large_frame/coarse_affine.npz');base={k:base[k] for k in base.files};oldpts=base['points']+shift;lookup={tuple(p):i for i,p in enumerate(oldpts)};original=np.array([tuple(p) in lookup for p in points]);original_index=np.array([lookup.get(tuple(p),-1) for p in points])
assert original.sum()==len(oldpts)==2184
SETTINGS=dict(previous_coarse_path='work/large_frame/coarse_affine.npz',previous_tracks_path='work/large_frame/ptv_tracks.npz',frozen_previous_coarse=2184,origin0=origin.tolist(),original_to_local_shift=shift.tolist(),coarse_radius=19,coarse_order=1,coarse_reg=.00005,poor_coarse_ncc=.7,coarse_valid_mindet=.15,coarse_valid_support=.7,coarse_fallback_search_dx=[-15,55],coarse_fallback_search_dy=[-20,20],coarse_fallback_candidates=3,detector_dog_sigmas=[.6,2],detector_local_max_width=5,detector_min_peak=8,detector_background_sigma=5,detector_background_max=180,detector_min_source_depth=14,detector_max_source_depth=maxdepth+25,detector_global_x=[4,int(rawA.shape[1]-5)],detector_patch_within_image=True,near_original_candidate_exclusion_radius=2.5,track_patch_radius=4,track_displacement_search_radius=8,track_min_common_fraction=.85,track_prior_penalty=.005,track_ncc_min=.68,track_reverse_ncc_min=.68,track_fb_max=1.,track_gap_min=.015,track_reverse_gap_min=.01,prior_neighbor_count=12,prior_spatial_sigma=25,prior_displacement_consensus_radius=4,manual_data_used=False,target_gradient_cache='exact optional cached np.gradient(B) and vb.astype(float); audit cached_solver_audit.json')
...
if (O/'coarse_affine.npz').exists():
    ...
    def coarse_task(i):
        if original[i]:
            j=original_index[i]
            return base['params'][j],{k:base[k][j].item() for k in ['ncc','rms','nvalid','cond','iterations','mindet','support_fraction']},int(base['coarse_fallback'][j])
        ...
        p,stats=fit_local(A,B,va,vb,pt,ini,r=19,order=1,reg=.00005,target_gradient=target_gradient,target_mask_float=target_mask_float)
        ...
        if fallback:
            for seed,score in translation_seed(A,B,va,vb,pt,r=19,search=(55,20)):
                ...
                pp,st=fit_local(A,B,va,vb,pt,seed,r=19,order=1,reg=.00005,target_gradient=target_gradient,target_mask_float=target_mask_float);fits.append((pp,st))
    ...
    co=dict(points=points,params=params,radius=19,original_coarse=original,original_coarse_index=original_index,frozen_previous_coarse=original,coarse_fallback=np.array([v[2] for v in result]),**stats)
...
old=np.load('work/large_frame/ptv_tracks.npz');old={k:old[k] for k in old.files};op=old['points']+shift;otree=cKDTree(op)
...
det=(hp==maximum_filter(hp,size=5))&(hp>8)&(xx>=4)&(xx<rawA.shape[1]-4)&(yy>=4)&(yy<rawA.shape[0]-4)&(depth>=14)&(depth<=maxdepth+25)&(va>.99)&(background<180)
...
TRACK_KEYS=['points','disp','prior','ncc','ambiguity_gap','back_disp','back_ncc','back_gap','fb','accepted','strength']
combined={k:np.concatenate([op if k=='points' else old[k],new[k]],axis=0) for k in TRACK_KEYS}
flag=np.r_[np.ones(len(op),bool),np.zeros(len(newpts),bool)]
combined.update(frozen_previous_candidate=flag,origin0=origin,source_depth=combined['points'][:,1]-np.interp(combined['points'][:,0],np.arange(len(sa)),sa))
assert np.allclose(combined['source_depth'][:len(op)],old['source_depth'],rtol=0,atol=1e-10)
combined['source_depth'][:len(op)]=old['source_depth']
for k in ['original_candidate','original_track','original_accepted_track']:
    combined[k]=np.r_[old[k],np.zeros(len(newpts),bool)]
for k in TRACK_KEYS:
    assert np.array_equal(combined[k][:len(op)],op if k=='points' else old[k]),k
for k in ['original_candidate','original_track','original_accepted_track']:
    assert np.array_equal(combined[k][:len(op)],old[k]),k
assert int(sum(combined['accepted'][:len(op)]))==3204
np.savez_compressed(O/'ptv_tracks.npz',**combined);np.savez_compressed(O/'new_ptv_tracks.npz',**new)
summary=dict(previous_candidate_rows=len(op),previous_accepted_tracks=int(sum(old['accepted'])),new_candidate_rows=len(newpts),new_accepted_tracks=int(sum(accepted)),total_candidate_rows=len(combined['points']),total_accepted_tracks=int(sum(combined['accepted'])),previous_rows_exactly_preserved_except_coordinate_shift=True,coarse_previous_preserved=2184,coarse_new_count=int(sum(~original)),coarse_good_prior_fits=int(sum(good)),new_accepted_min_depth=float(np.min(combined['source_depth'][len(op):][accepted])) if accepted.any() else None,total_seconds=time.time()-t0)
